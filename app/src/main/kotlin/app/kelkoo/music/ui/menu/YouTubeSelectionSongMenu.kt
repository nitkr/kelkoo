    ...
    AddToPlaylistDialogOnline(
        ...
        songs = remember { 
            songSelection.map { song ->
                ...
                val metadata = song.toMediaMetadata()
                app.kelkoo.music.db.entities.Song(
                    song = app.kelkoo.music.db.entities.SongEntity(
                        id = metadata.id,
                    ...
                    artists = metadata.artists.map { artist ->
                        app.kelkoo.music.db.entities.ArtistEntity(
                            id = artist.id ?: "",
                            name = artist.name
                        )
                    },
                    album = metadata.album?.let { album ->
                        app.kelkoo.music.db.entities.AlbumEntity(
                            id = album.id,
                            title = album.title,
                            thumbnailUrl = metadata.thumbnailUrl, 
                            songCount = 0,
                            ...
                            database.transaction {
                                songSelection.forEach { song ->
                                    ...
                                    if ((!allLiked && !metadata.liked) || allLiked) {
                                        ...
                                        val songEntity = app.kelkoo.music.db.entities.SongEntity(
                                            id = metadata.id,
                                            title = metadata.title,
                                            duration = metadata.duration,
                                            thumbnailUrl = metadata.thumbnailUrl,
