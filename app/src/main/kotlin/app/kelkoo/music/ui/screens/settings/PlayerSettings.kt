...
import app.kelkoo.music.constants.AudioQuality
...
import app.kelkoo.music.constants.AutomixDebugOverlayKey
...
import app.kelkoo.music.constants.EnableGoogleCastKey
import app.kelkoo.music.constants.HistoryDuration
import app.kelkoo.music.constants.KeepScreenOn
...
import app.kelkoo.music.constants.SeekExtraSeconds
...
import app.kelkoo.music.constants.StopMusicOnTaskClearKey
import app.kelkoo.music.constants.EnableExportAsMp3Key
...
highlightKey: String? = null) {
    ...
    val (dataSaverEnabled, onDataSaverEnabledChange) = rememberPreference(
        key = app.kelkoo.music.constants.DataSaverEnabledKey,
        defaultValue = false
    )
    ...
    val (playbackEngine, onPlaybackEngineChange) = rememberEnumPreference(
        app.kelkoo.music.constants.PlaybackEngineKey,
        defaultValue = app.kelkoo.music.constants.PlaybackEngine.AUTO
    )

    val (downloadQuality, onDownloadQualityChange) = rememberEnumPreference(
        app.kelkoo.music.constants.DownloadQualityKey,
        defaultValue = app.kelkoo.music.constants.DownloadQuality.YOUTUBE
    )
    ...
    if (showDownloadQualityDialog) {
        EnumDialog(
            ...
            values = listOf(app.kelkoo.music.constants.DownloadQuality.YOUTUBE),
            valueText = {
                when (it) {
                    app.kelkoo.music.constants.DownloadQuality.YOUTUBE -> "YouTube Music (AAC/Default)"
                    else -> ""
                }
            }
        )
    ...
    if (showPlaybackEngineDialog) {
        EnumDialog(
            ...
            onSelect = {
                ...
                app.kelkoo.music.utils.YTPlayerUtils.playbackEngine = it
                showPlaybackEngineDialog = false
            },
            title = "Playback Engine",
            current = playbackEngine,
            values = listOf(
                app.kelkoo.music.constants.PlaybackEngine.POTOKEN,
                app.kelkoo.music.constants.PlaybackEngine.BRAVEPIPE,
                app.kelkoo.music.constants.PlaybackEngine.AUTO,
            ),
            valueText = {
                when (it) {
                    app.kelkoo.music.constants.PlaybackEngine.POTOKEN -> "PoToken (Recommended)"
                    app.kelkoo.music.constants.PlaybackEngine.BRAVEPIPE -> "BravePipe (NewPipe)"
                    app.kelkoo.music.constants.PlaybackEngine.AUTO -> "Auto (Try Both)"
                }
            },
            valueDescription = {
                when (it) {
                    app.kelkoo.music.constants.PlaybackEngine.POTOKEN -> "Uses WebView PoToken + CipherDeobfuscator. Most reliable and future-proof."
                    app.kelkoo.music.constants.PlaybackEngine.BRAVEPIPE -> "Uses NewPipe extractor for stream resolution. Lightweight but may break with YouTube updates."
                    app.kelkoo.music.constants.PlaybackEngine.AUTO -> "Tries PoToken first, falls back to BravePipe if it fails."
                }
            }
        )
    }
    ...
    isHighlighted = (highlightKey == stringResource(R.string.seek_seconds_addup)),
                ...
                add(Material3SettingsItem(
    isHighlighted = (highlightKey == stringResource(R.string.axion_equalizer)),
                    icon = painterResource(R.drawable.axion_equalizer),
                    title = { Text(stringResource(R.string.axion_equalizer)) },
                    description = { Text(stringResource(R.string.axion_equalizer_desc)) },
